...
void Mesh::initaliseQuad()
{
	...
	verties[0].tangent = {1,0,0,1};
	verties[1].tangent = {1,0,0,1};
	verties[2].tangent = {1,0,0,1};
	verties[3].tangent = {1,0,0,1};
	...
	glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)sizeof(glm::vec4)); // needs to have tyhe correct stride
	//UV enable
	...
	//unbind buffers
	glBindVertexArray(0);
	glBindBuffer(GL_ARRAY_BUFFER, 0); //binds the buffer to 0 
